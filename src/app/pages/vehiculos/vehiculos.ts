...
export class Vehiculos implements OnInit {
  ...
  private readonly PLACA_REGEX = /^[A-Z]{3}[\s_-]*[0-9]{3}$/;
  ...
  saveVehiculo(): void {
    ...
    if (!this.PLACA_REGEX.test(placaNormalizada)) {
      this.error.set('La placa debe tener formato ABC123, ABC 123, ABC-123 o ABC_123 (3 letras y 3 numeros).');
      return;
    }
  ...
  private normalizePlaca(value: string): string {
    return (value || '').toUpperCase().replace(/[^A-Z0-9\s_-]/g, '').trim();
  }
